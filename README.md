...
2. **访问前端页面**：在浏览器中打开 `http://localhost:8001/app.html`

3. **页面功能**：
   - **左侧栏**：显示Sonetto的指导内容（来自communication.md的"### 下一步"部分）
   - **右侧栏**：根据communication.md中的三级标题生成对应的输入框
   - **底部按钮**：
     - "下一步"：提交输入内容，相当于终端中输入"go"
     - "结束"：结束会话，相当于终端中输入"exit"

4. **实时更新**：页面会每5秒自动检查communication.md文件的更新，确保显示最新内容
